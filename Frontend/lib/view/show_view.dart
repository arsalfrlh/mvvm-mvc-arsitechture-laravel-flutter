...
class ShowView extends StatefulWidget {
  const ShowView({required this.barangId});
  final int barangId;
...
class _ShowViewState extends State<ShowView> {
  ...
  void initState() {
    ...
    barangViewmodel.showBarang(widget.barangId);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: barangViewmodel.isLoading || barangViewmodel.barang == null
      ? Center(child: CircularProgressIndicator(),)
      : ListView(
          children: [
            ProductImages(barang: barangViewmodel.barang!),
            TopRoundedContainer(
              color: Colors.white,
              child: Column(
                children: [
                  ProductDescription(
                    barang: barangViewmodel.barang!,
                    pressOnSeeMore: () {},
                  ),
                  TopRoundedContainer(
                    ...
                    child: Column(
                      children: [
                        ColorDots(barang: barangViewmodel.barang!),
                      ],
                    ),
                  ),
                ],
    ...
    return Column(
      children: [
        SizedBox(
          ...
          child: AspectRatio(
            aspectRatio: 1,
            child: CachedNetworkImage(imageUrl: "http://192.168.0.100:8000/storage/images/${widget.barang.gambar}", fit: BoxFit.cover, errorWidget: (context, url, error) => Icon(Icons.broken_image, size: 120,),),
          ),
        ),
      ],
    );
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: Text(
            "${barang.merk} ${barang.namaBarang} ini dijual dengan harga Rp: ${barang.harga}, dan stok tersisa: ${barang.stok} lagi",
            maxLines: 3,
          ),
        ),
        Padding(
