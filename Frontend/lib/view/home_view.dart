...
import 'package:toko/view/tambah_view.dart';
import 'package:toko/view/update_view.dart';
...
class _HomeViewState extends State<HomeView> {
  ...
  @override
  void initState() {
    Provider.of<BarangViewmodel>(context, listen: false).fetchBarang();
    super.initState();
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: (){
            Navigator.push(context, MaterialPageRoute(builder: (context) => TambahView())).then((_) => barangViewmodel.fetchBarang());
          }, icon: Icon(Icons.add))
        ],
      ),
      body: RefreshIndicator(
        onRefresh: barangViewmodel.fetchBarang,
          ...
          child: Padding(
            ...
            child: GridView.builder(
              ...
              itemBuilder: (context, index) => ProductCard(
                barang: barangViewmodel.barangList[index],
                onPress: () {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => ShowView(barangId: barangViewmodel.barangList[index].id)));
                },
                onUpdate: () {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => UpdateView(barang: barangViewmodel.barangList[index]))).then((_) => barangViewmodel.fetchBarang());
                },
                onDelete: () {
                  AwesomeDialog(
                    context: context,
                    ...
                    btnOkOnPress: (){
                      barangViewmodel.deleteBarang(barangViewmodel.barangList[index].id);
                      barangViewmodel.fetchBarang();
                    },
          ...
          children: [
            AspectRatio(
              ...
              child: Container(
                ...
                child: CachedNetworkImage(imageUrl: "http://192.168.0.100:8000/storage/images/${barang.gambar}", fit: BoxFit.cover, errorWidget: (context, url, error) => Icon(Icons.broken_image, size: 120,),),
              ),
            ),
