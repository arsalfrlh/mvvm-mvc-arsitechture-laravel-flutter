...
import 'package:toko/view/register_view.dart';
...
class _LoginViewState extends State<LoginView> {
  ...
  Widget build(BuildContext context) {
    final authViemodel = Provider.of<AuthViewmodel>(context);
    return Scaffold(
                  ...
                  SignInForm(
                    ...
                    login: ()async{
                      if(emailController.text.isNotEmpty && passwordController.text.isNotEmpty){
                        ...
                        final response = await authViemodel.login(emailController.text, passwordController.text);
                        ...
                        if(response){
                          AwesomeDialog(
                            ...
                            title: "Sukses",
                            desc: authViemodel.message,
                            btnOkOnPress: (){
                              Navigator.pushAndRemoveUntil(
                                context, MaterialPageRoute(builder: (context) => HomeView()), (route) => false);
                            }
                          ).show();
                        }else{
                          AwesomeDialog(
                            ...
                            dismissOnTouchOutside: false,
                            title: "Error",
                            desc: authViemodel.message.toString(),
                            btnOkOnPress: (){},
    ...
    return Form(
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          ElevatedButton(
            onPressed: authViemodel.isLoading ? null : login,
            ...
            child: authViemodel.isLoading ? CircularProgressIndicator() : Text("Continue"),
          )
        ],
      ),
    );
